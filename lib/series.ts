...
export async function getAllSeries(): Promise<SeriesInfo[]> {
  ...
  for (const post of allPosts) {
    if (post.series) {
      const seriesName = post.series.toLowerCase().trim()
  ...
  const series: SeriesInfo[] = Array.from(seriesMap.entries()).map(
    ([name, posts]) => {
      ...
      // 첫 번째 포스트의 커버 이미지 사용
      const coverImage = sortedPosts[0]?.coverImage
 ...
 * 특정 시리즈 정보 가져오기
 */
export async function getSeriesInfo(series: string): Promise<SeriesInfo | null> {
...
export async function getSeriesNavigation(
  ...
  return {
    previous:
      currentIndex > 0 ? seriesInfo.posts[currentIndex - 1] : null,
    next:
 ...
 * 인기 시리즈 가져오기 (상위 N개)
 */
export async function getPopularSeries(limit: number = 5): Promise<SeriesInfo[]> {
  const allSeries = await getAllSeries()
 ...
 * 최근 시작된 시리즈 가져오기
 */
export async function getRecentSeries(limit: number = 5): Promise<SeriesInfo[]> {
  const allSeries = await getAllSeries()
